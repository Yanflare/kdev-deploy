...
from textual.widgets import DataTable, Input, Label, ListView, ListItem, RichLog, Static
...
class KDEVApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        # Input docked to bottom (declared before content so it docks first)
    ...
    def on_input_changed(self, event: Input.Changed) -> None:
        if event.value.startswith("/"):
            self._show_slash_menu(event.value)
        else:
            self._hide_slash_menu()
