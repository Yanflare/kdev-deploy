...
TURBO_PORT = 8082
MODEL_PATH = "/usr/share/ollama/.ollama/models/blobs/sha256-29ed0ac6f51294cdfcbdf77535162e751fac1fe7ebc87cb2893fb70f9e7b5edb"
# ===============================================

def log(msg):
    ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"[{ts}] TURBOQUANT_SETUP_v9: {msg}")
...
def main():
    log("=== KDEV TurboQuant Setup Phase 2B.4 v9 (EXACT PATH) STARTED ===")
    
    # Fix permissions so yanflare can read the ollama-owned blob
    log("Fixing permissions on model blob...")
    run(["sudo", "chmod", "a+r", MODEL_PATH])
    
    # Stop any previous service
    run(["sudo", "systemctl", "stop", "kdev-turboquant.service"], check=False)
    
    # Install the final clean service
    log("Installing TurboQuant sidecar with exact model path...")
