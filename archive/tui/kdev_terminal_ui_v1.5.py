#!/usr/bin/env python3
"""
KDEV TERMINAL UI v1.5 — Cyberpunk KDE Operator Console
"""
...
class KDEVApp(App):
    ENABLE_COMMAND_PALETTE = False   # ← kills the ? help overlay and ^p palette forever
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container():
            ...
            # NEW BOTTOM COMMAND CHAT PANEL
            with Vertical(classes="panel"):
                yield Static("COMMAND CHAT LOG — Messages sent to Orchestrator", classes="logo")
                self.command_log = RichLog(highlight=True, wrap=True, auto_scroll=True, max_lines=50)
                yield self.command_log

        yield Input(placeholder="Type message → send to 9B Orchestrator", id="cmd_input")
        yield Footer()
    ...
    def on_input_submitted(self, event):
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                self.command_log.write(f"[#ff0033]← ORCHESTRATOR: {final[:120]}[/]")
                self.logs.write(f"[#ff0033]ORCHESTRATOR RESPONSE: {final[:80]}…[/]")
            else:
                self.command_log.write(f"[#ff0033]← ORCHESTRATOR: (no response or error)[/]")
        except Exception as e:
            self.command_log.write(f"[#ff0033]← ERROR sending to orchestrator: {e}[/]")
