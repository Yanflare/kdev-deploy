#!/usr/bin/env python3
"""
KDEV TERMINAL UI v1.9 — Cyberpunk KDE Operator Console
"""
...
class KDEVApp(App):
    ...
    BINDINGS = [("?", "noop", None), ("escape", "restore_layout", "Restore layout")]
    ...
    system_healthy = reactive(True)
    maximized_panel = reactive(None)
    ...
    def on_button_pressed(self, event):
        ...
        if panel_id:
            ...
            for p in self.query(".panel"):
                p.styles.display = "none" if p.id != panel_id else "block"

    def action_restore_layout(self):
    ...
    # update_corpus, update_status, load_recent_traces, live_log_stream, live_corpus_watcher, kairos_heartbeat, on_input_submitted are the same as v1.8
