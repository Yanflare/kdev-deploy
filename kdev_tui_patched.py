...
class KDEVApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="content_area"):
            ...
            with Vertical(id="chat_zone"):
                ...
                self.command_log = RichLog(highlight=True, markup=True, wrap=True,
    ...
    async def live_health_checker(self) -> None:
        while True:
            ...
            if prev != healthy:
                if healthy:
                    self.command_log.write("[#00ff9d]>> SYSTEM BACK ONLINE[/]")
                else:
                    self.command_log.write(
    ...
    async def _send_to_orchestrator(self, cmd: str) -> None:
        try:
            ...
            if not KDEVApp._cookie_ready:
                ...
                if not ok:
                    self.command_log.write("[#ff0033]<- KDEV: login failed[/]")
                    return
            ...
            def redraw():
                """Clear log and rewrite all history + growing response."""
                self.command_log.clear()
                for entry in self._log_history:
                    self.command_log.write(entry)
                current = "".join(assembled)
                self.command_log.write(
                    f"[#ff0033]<- KDEV: {current}[/]"
                )
            ...
            if got_401 or not assembled:
                ...
                ok = await loop.run_in_executor(None, self._do_login)
                if not ok:
                    self.command_log.write("[#ff0033]<- KDEV: re-login failed[/]")
                    return
                assembled.clear()
                got_401 = False
                await loop.run_in_executor(None, lambda: stream_to_ui(self._cookie_path))
            ...
            self._log_history.append(f"[#ff0033]<- KDEV: {final}[/]")
